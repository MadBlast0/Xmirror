...
function Get-BeaconPython {
    ...
    # A virtual environment's python.exe inherits the venv's own site-packages,
    # not the interpreter's, so installing the beacon's requirements into one
    # fails (and would pollute an unrelated venv). Skip anything that sits next
    # to a pyvenv.cfg and take the real interpreter instead.
    $windowsPython = Get-Command python.exe -All -ErrorAction SilentlyContinue |
        Where-Object {
            ...
            -not (Test-Path -LiteralPath (Join-Path (Split-Path -Parent (Split-Path -Parent $_.Source)) "pyvenv.cfg"))
        } |
...
function Build-Application {
    ...
    $beaconSource = Join-Path $projectRoot "libxmirror\Bluetooth_LE_beacon"
...
function Write-BuildManifest {
    ...
    # Version metadata is informational. A source tree with no git history
    # (a plain ZIP download, or a fresh checkout-less copy) must still build.
    $sourceRef = & git -C $projectRoot branch --show-current 2>$null
    if ($sourceRef) {
        $sourceRef = $sourceRef.Trim()
    } elseif ($env:GITHUB_REF_NAME) {
        $sourceRef = $env:GITHUB_REF_NAME.Trim()
    } else {
        $shortRef = & git -C $projectRoot rev-parse --short HEAD 2>$null
        $sourceRef = if ($shortRef) { $shortRef.Trim() } else { "unversioned" }
    }

    $commitRef = & git -C $projectRoot rev-parse HEAD 2>$null
    $commitRef = if ($commitRef) { $commitRef.Trim() } else { "unversioned" }
...
function Stage-Runtime {
    ...
    Copy-Item `
        (Join-Path $buildDir "XMirror.exe") `
        (Join-Path $stageDir "XMirror.exe") `
        -Force
    Copy-Item `
        (Join-Path $beaconOutDir "dist\xmirror-bluetooth-beacon.exe") `
        (Join-Path $stageDir "xmirror-bluetooth-beacon.exe") `
        -Force
    Copy-Item `
    ...
    Copy-Item `
        ...
        -Force
    Copy-Item `
        (Join-Path $projectRoot "stuff\XMirror.ico") `
        (Join-Path $stageDir "resources\icon.ico") `
        -Force
    Copy-Item `
        (Join-Path $projectRoot "stuff\xmirror_arguments_list.txt") `
        (Join-Path $stageDir "resources\xmirror_arguments_list.txt") `
        -Force
    Copy-Item `
        $featuresFile `
...
function Build-Artifacts {
    ...
    $zip = Join-Path $artifactDir "XMirror-$Architecture-portable.zip"
    ...
    if (-not $SkipInstaller) {
        ...
        $msi = Join-Path $artifactDir "XMirror-$Architecture.msi"
