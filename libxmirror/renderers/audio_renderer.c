...
void audio_renderer_init(logger_t *render_logger, const char* audiosink, const bool* audio_sync, const bool* video_sync, const char *artp_pipeline) {
    ...
    for (int i = 0; i < NFORMATS ; i++) {
        ...
        /* Keep this queue. Removing it entirely was measured on 27 Aug and made
         * audio *worse* (185 -> 205-213 ms): without the thread boundary the
         * blocking sink write back-pressures the RTP receive thread. Bounding
         * it was measured earlier and also gave nothing (171 -> 180 ms). Both
         * directions are now closed; the default is the best of the three. */
        ...
        if (!audio_rtp) {
            ...
            /* "level" removed: it computes RMS/peak for every buffer and posts
             * a bus message per interval, and the bus handler discards them
             * (GST_MESSAGE_ELEMENT, below). Pure per-buffer cost and bus
             * traffic for output nobody reads. Restore it here if a VU meter is
             * ever added to the GUI. */
