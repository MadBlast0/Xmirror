 ...
 * Modified for:
 * XMirror - An AirPlay mirroring server for Windows
 * Modifications Copyright (C) 2026 MadBlast0
 *
...
// Written by stop_xmirror() from another thread, read on the engine thread.
// Plain bool lets the optimiser cache the read, so a stop request is never
// observed and the engine runs forever.
...
// Also written cross-thread by stop_xmirror(); see reset_loop above.
...
static gboolean reset_callback(gpointer loop) {
    // do_shutdown is checked here as well as reset_loop: main_loop() clears
    // reset_loop on entry, so a stop requested while the engine was still
    // starting up would otherwise be discarded and the loop would run forever.
    if (reset_loop || do_shutdown) {
        g_main_loop_quit((GMainLoop *) loop);
    }
    return TRUE;
}
...
static std::string find_xmirror_config_file() {
    std::string no_config_file = "";
    const char *homedir = NULL;
    const char *xmirrorrc = NULL;
    std::string config0, config1, config2;
    struct stat sb;
    xmirrorrc = getenv("XMIRRORRC");   /* first look for $XMIRRORRC */
    if (xmirrorrc) {
        config0 = xmirrorrc;
    ...
    if (homedir) {
      config1 = homedir;
      config1.append("/.xmirrorrc");
      if (stat(config1.c_str(), &sb) == 0) return config1;  /* look for ~/.xmirrorrc */
      config2 = homedir;
      config2.append("/.config/xmirrorrc"); /* look for ~/.config/xmirrorrc */
...
static void print_info (char *name) {
    printf("XMirror %s: An open-source AirPlay mirroring server.\n", VERSION);
    printf("=========== Website: https://github.com/MadBlast0/XMirror ==========\n");
    ...
    printf("-reg [fn] Keep a register in $HOME/.xmirror.register to verify returning\n");
    ...
    printf("          XMirror displays deviceID when a client attempts to connect\n");
    ...
    printf("-m [mac]  Set MAC address (also Device ID);use for concurrent XMirrors\n");
    printf("          if mac xx:xx:xx:xx:xx:xx is not given, a random MAC is used\n");
    printf("-key [fn] Store private key in $HOME/.xmirror.pem (or in file \"fn\")\n");
    printf("-dacp [fn]Export client DACP information to file $HOME/.xmirror.dacp\n");
    printf("          (option to use file \"fn\" instead); used for client remote\n");
    printf("-ble [fn] For BluetoothLE beacon: write data to file ~/.xmirror.ble\n");
    ...
    printf("-rc fn    Read startup options from file \"fn\" instead of ~/.xmirrorrc, etc\n");
    printf("Startup options in $XMIRRORRC, ~/.xmirrorrc, or ~/.config/xmirrorrc are\n");
...
static void read_config_file(const char * filename, const char * xmirror_name) {
    std::string config_file = filename;
    std::string option_char = "-";
    std::vector<std::string> options;
    options.push_back(xmirror_name);
    std::ifstream file(config_file);
    if (file.is_open()) {
        fprintf(stdout,"XMirror: reading configuration from  %s\n", config_file.c_str());
        std::string line;
        ...
        fprintf(stderr,"XMirror: failed to open configuration file at %s\n", config_file.c_str());
    }
    if (options.size() > 1) {
...
int start_xmirror (int argc, char *argv[]) {
    ...
    if (rcfile) {
        ...
        config_file = find_xmirror_config_file();
    }
    ...
    LOGI("XMirror %s: An Open-Source AirPlay mirroring and audio-streaming server.", VERSION);
    ...
    if (!new_window_closing_behavior) {
        LOGI("XMirror on macOS is using -nc option as workaround for GStreamer problem: use \"-nc no\" to omit workaround");
    }
#endif
    ...
    if (pin_pw == 1 && registration_list) {
        if (pairing_register == "") {
            ...
            if (homedir) {
                ...
                pairing_register.append("/.xmirror.register");
             }
        }
    }
...
#define PID_MAX 4194304 // 2^22
    ...
    cleanup();
    // This path (no relaunch, no shutdown flag) previously fell off the end of
...
void stop_xmirror() {
    ...
    //
    // Only raise flags here. Tearing down the RAOP server and dnssd from a
    // foreign thread frees objects that the engine's still-running main loop is
    // actively using, and quitting a GMainLoop through the stale g_loop pointer
    // (main_loop() creates a fresh loop per session and never clears g_loop)
    // compounds it. Both together take the whole process down.
    //
    // reset_callback() already polls reset_loop every 100 ms on the engine's own
    // thread and quits the loop from there; the code after the loop then checks
    // do_shutdown and performs its own teardown. So flags are sufficient, and
    // everything destructive happens on the thread that owns it.
    do_shutdown = true;
    reset_loop = true;
}
...
#endif
    // No exit() here. This is built as a library: terminating the process from
    // cleanup() takes the whole host application down on every engine shutdown.
    // The standalone CtrlHandler paths still call exit(0) explicitly, and every
    // error path below now returns a non-zero status instead.
}
