...
#----------------------------------------------------------------
# a standalone python-3.6 or later AirPlay Service-Discovery Bluetooth LE beacon for XMirror 
# (c)  F. Duncanh, October 2025

import sys
if not sys.version_info >= (3,6):
    print("xmirror-beacon.py requires Python 3.6 or higher")
        ...
        if test:
            if not beacon_is_running:
                ...
                if not check_port(port):
                    # xmirror is active, and beacon is running but is advertising a different port, so shut it down
...
def parse_params():
    ...
    default_config_file = home_dir+"/.xmirror.beacon"
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='beacon startup file (Default: ~/.xmirror.beacon).'
    )
    
    parser.add_argument(
        '--path',
        type=str,
        default= home_dir + "/.xmirror.ble", 
        help='path to AirPlay server BLE beacon information file (default: ~/.xmirror.ble).'
    )
    parser.add_argument(
        '--ipv4',
        type=str,
    ...
    if config_file is not None:
        print("Read xmirror-beacon.py configuration file ", config_file)
        try:
...
if __name__ == '__main__':
    ...
    module = f'xmirror_beacon_module_{ble_type}'
